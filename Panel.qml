...
Panel {
  ...
  // Geometry/orientation for the ARRANGE section. `omarchy-monitor-state`
  // only reports name/enabled/focused/size, so this comes from a second
  // query that adds position, transform and rotation-aware logical size.
  property var arrangement: []

  // Displays are placed relative to one anchor: the built-in panel when
  // there is one, otherwise the first display. The anchor itself never
  // moves, so the arrangement can't drift away from the origin.
  readonly property var anchorDisplay: {
    var list = root.arrangement || []
    if (list.length < 2) return null
    for (var i = 0; i < list.length; i++) {
      if (list[i].internal) return list[i]
    }
    return list[0]
  }
  ...
  function refresh() {
    ...
    if (!arrangeStateProc.running) arrangeStateProc.running = true
  }
  ...
  // Which side of the anchor this display currently sits on, so the matching
  // pill can render as active. Returns "" for overlapping/diagonal layouts.
  function sideOf(display) {
    var anchor = root.anchorDisplay
    if (!anchor || !display || anchor.name === display.name) return ""
    if (display.x + display.logicalWidth <= anchor.x) return "left"
    if (display.x >= anchor.x + anchor.logicalWidth) return "right"
    if (display.y + display.logicalHeight <= anchor.y) return "above"
    if (display.y >= anchor.y + anchor.logicalHeight) return "below"
    return ""
  }

  function placeDisplay(name, side) {
    var anchor = root.anchorDisplay
    if (!anchor || !name || anchor.name === name) return
    layoutProc.command = ["nvdk-display-layout", "place", name, side, anchor.name]
    if (!layoutProc.running) layoutProc.running = true
  }

  function rotateDisplay(name, degrees) {
    if (!name) return
    layoutProc.command = ["nvdk-display-layout", "rotate", name, String(degrees)]
    if (!layoutProc.running) layoutProc.running = true
  }
  ...
  Process {
    id: arrangeStateProc
    command: ["nvdk-display-layout", "state"]
    stdout: StdioCollector {
      waitForEnd: true
      onStreamFinished: {
        try {
          var parsed = JSON.parse(String(text || "[]"))
          root.arrangement = Array.isArray(parsed) ? parsed : []
        } catch (e) {
          root.arrangement = []
        }
      }
  ...
  // Placement and rotation both apply live and rewrite the managed block in
  // monitors.lua, so the change survives a reload. Re-read state when done.
  Process {
    id: layoutProc
    stdout: StdioCollector { waitForEnd: true }
  ...
  KeyboardPanel {
    ...
    // No explicit cap: grow to fit the content so the ARRANGE rows don't
    // land behind a scrollbar. fittedContentHeight still clamps to the
    // screen-derived availableCardHeight, so this can't overflow the display.
    ...
    PanelKeyCatcher {
      ...
      ScrollView {
        ...
        Column {
          ...
          // ---------- Arrange (position + orientation) ----------
          PanelSeparator {
            visible: root.arrangement.length > 1
            foreground: root.bar.foreground
          }
  ...
  component ArrangeBlock: Column {
    id: block
    required property var display

    readonly property bool isAnchor: root.anchorDisplay
      && root.anchorDisplay.name === block.display.name

    spacing: Style.space(6)

    Item {
      width: parent.width
      implicitHeight: Math.max(blockName.implicitHeight, blockSize.implicitHeight)

      Text {
        id: blockName
        text: block.display.name + (block.isAnchor ? " · anchor" : "")
        color: root.bar.foreground
        font.family: root.bar.fontFamily
        font.pixelSize: Style.font.body
        elide: Text.ElideRight
      ...
      // Logical size, so a rotated display reads 1440×2560 rather than the
      // unrotated mode it still reports.
      Text {
        id: blockSize
        text: block.display.logicalWidth + "×" + block.display.logicalHeight
        color: Qt.darker(root.bar.foreground, 1.4)
        font.family: root.bar.fontFamily
        font.pixelSize: Style.font.caption
        anchors.right: parent.right
    ...
    Grid {
      ...
      Repeater {
        model: [
          ...
          { label: "Above", side: "above" },
          { label: "Below", side: "below" }
        ]
        ...
        LayoutPill {
          ...
          width: placeRow.cellWidth
          active: root.sideOf(block.display) === modelData.side
          onClicked: root.placeDisplay(block.display.name, modelData.side)
        }
      }
    }
    ...
    Grid {
      ...
      Repeater {
        model: [
          { label: "0°", transform: 0, degrees: 0 },
          { label: "90°", transform: 1, degrees: 90 },
          { label: "180°", transform: 2, degrees: 180 },
          { label: "270°", transform: 3, degrees: 270 }
        ]
        ...
        LayoutPill {
          ...
          text: modelData.label
          width: rotateRow.cellWidth
          active: block.display.transform === modelData.transform
          onClicked: root.rotateDisplay(block.display.name, modelData.degrees)
        }
      }
    }
  }
